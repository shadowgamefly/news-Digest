...
def parse_article(url, count):
    ...
    article_name = tree.xpath('//h1[@class="c-page-title"]/text()')[0]
    art = {
        'name': str(pk),
        'parent': '',
        'title': article_name,
...
def parse_image(url, count):
    ...
    imgs = tree.xpath('//figure[@class="e-image"]')
    imgs_json = []
...
def parse(href):
    global pk
    pk += 1
    count = parse_comment('http://www.theverge.com/' + href)
    count = parse_article('http://www.theverge.com/' + href, count)
    parse_image('http://www.theverge.com/' + href, count)


if __name__ == '__main__':
    pk = 0
    parse('2017/3/24/15049350/life-movie-review-daniel-espinosa-jake-gyllenhaal-ryan-reynolds-space-horror')
